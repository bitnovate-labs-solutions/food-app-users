...
export default function TreateeDetails({ item, isOpen, onClose }) {
  ...
  const { cachedUrl: packageImageUrl, isImageLoaded: packageImageLoaded } =
    useImageCache(item.menu_images?.[0]?.image_url);
  const {
    cachedUrl: restaurantImageUrl,
    isImageLoaded: restaurantImageLoaded,
  } = useImageCache(item?.restaurant?.image_url);
      ...
      <DialogContent className="sm:max-w-[600px] bg-white border-none shadow-xl rounded-2xl max-h-[90vh] overflow-y-auto pb-8">
        ...
        <div className="space-y-6">
          ...
          <div className="relative w-full h-[250px] rounded-lg overflow-hidden">
            <ImageWithFallback
              src={
                isPackageImageLoaded
                  ? packageImageUrl || defaultImage
                  : defaultImage
              }
              alt={item.name}
              ...
              <ImageWithFallback
                src={
                  isRestaurantImageLoaded
                    ? restaurantImageUrl || defaultImage
                    : defaultImage
                }
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <div>
                <p className="font-semibold mb-1">{item?.restaurant?.name}</p>
