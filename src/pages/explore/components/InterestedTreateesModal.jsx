...
export default function InterestedTreateesModal({
  ...
  const handleSendMessage = async () => {
    try {
      ...
      // selectedTreatee.profile_id is the profile ID (which equals auth.users.id)
      if (!user?.id || !selectedTreatee?.profile_id || !purchaseId) {
        console.error("Missing required data for chat request:", {
          userId: user?.id,
          treateeId: selectedTreatee?.profile_id,
          purchaseId,
        });
        return;
      }
      ...
      // Message functionality removed - Collection page replaces Messages
      // if (conversation?.id) {
